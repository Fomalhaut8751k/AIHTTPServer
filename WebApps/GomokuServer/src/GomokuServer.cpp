...
void GomokuServer::setSessionManager(std::unique_ptr<http::session::SessionManager> manager)
{
    httpServer_.setSeesionManager(std::move(manager));
}

http::session::SessionManager* GomokuServer::getSessionManager() const
{
...
void GomokuServer::getBackendData(const http::HttpRequest& req, http::HttpResponse* resp)
{
    try
    {
        ...
        logger_->INFO(std::string("当前在线人数: ") + std::to_string(curOnline));

        int maxOnline = getMaxOnline();
        logger_->INFO(std::string("历史最高在线人数: ") + std::to_string(maxOnline));

        int totalUser = getUserCount();
        logger_->INFO(std::string("已注册用户总数: ") + std::to_string(totalUser));
        ...
        logger_->INFO("Backend data response prepared successfully");
...
void GomokuServer::packageResp(const std::string& version, http::HttpResponse::HttpStatusCode statusCode, 
...
// 获取历史最高在线人数
int GomokuServer::getMaxOnline() const
{
    return maxOnline_;
}
...
void GomokuServer::updateMaxOnline(int online)
{
