...
void RegisterHandler::handle(const http::HttpRequest& req, http::HttpResponse* resp)
{
    ...
    std::string invitationCode = parsed["invitationCode"];

    if(invitationCode != ChatServer::registerInvitationCode){
        // 邀请码错误
        json failureResp;
        failureResp["status"] = "error";
        failureResp["message"] = "register invitation code is error";
...
{
    // 判断用户是否存在，如果不存在则返回-1，否则返回用户id
    if(!isUserExist(username))
    {
        // 用户不存在，插入用户
        std::string sql = "INSERT INTO User (name, password) VALUES ('" + username + "', '" + password + "')";
        mysqlUtil_.executeUpdate(sql);
        std::string sql2 = "SELECT id FROM User WHERE name = '" + username + "'";
        sql::ResultSet* res = mysqlUtil_.executeQuery(sql2);
        if(res->next())
        {   // 先插入数据，然后找到对应生成的id并返回
