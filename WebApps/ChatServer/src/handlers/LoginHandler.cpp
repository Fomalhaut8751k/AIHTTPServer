...
void LoginHandler::handle(const http::HttpRequest& req, http::HttpResponse* resp)
{
    ...
    {
        logger_->INFO("content" + req.getBody());
    ...
    try
    {
        ...
        if(userId != -1)
        {
            ...
            /*
                ...
                所有不同页面的访问不可能是相同会话的，只有该页面前面访问过服务端，才会有会话记录
                于是，判断用户是否在其他地方登录中就不能通过会话来判断
            */
        ...
        else  // 账号密码错误，请重新登录
        {
            ...
            resp->setCloseConnection(false);
            resp->setContentType("application/json");
            resp->setContentLength(failureResp);
            return;
        }
...
int LoginHandler::queryUserId(const std::string &username, const std::string &password)
{
    ...
    // 使用预处理语句，防止sql注入
    std::string sql = "SELECT id FROM User WHERE name = ? AND password = ?";
